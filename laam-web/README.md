...
`laam-web`은 `laam` QR 메뉴 프로젝트의 손님용 모바일 웹 프론트엔드입니다.
...
메뉴 주문 및 일반·특별 요청 접수에는 `laam-web/.env.local`에 API가 공유할 내부 토큰을 설정합니다.

```bash
PAYMENT_API_TOKEN=laam-api와_동일한_긴_임의값
```
...
토스플레이스 키는 반드시 `laam-api`에서만 관리합니다. 메뉴의 주문 버튼은 결제창을 열지 않고 후불 주문을 POS에 등록하며, 결제는 매장에서 진행합니다.
